...
struct f2fs_inode_info {
	...
	// 将脏的inode放入到全局inode_hashtable链表
...
struct f2fs_nm_info {
	...
	// 下一个将要被扫面的node id，用于轮询NAT区域（为了磨损均衡？）
	nid_t next_scan_nid;		/* the next nid to be scanned */
	nid_t max_rf_node_blocks;	/* max # of nodes for recovery */
	unsigned int ram_thresh;	/* control the memory footprint */
	// 用于异步预读下一次需要遍历的nat block page数
	unsigned int ra_nid_pages;	/* # of nid pages to be readaheaded */
	unsigned int dirty_nats_ratio;	/* control dirty nats ratio threshold */

	/* NAT cache management */
	// 缓存部分nat_entry，用于加快查找效率，避免总是要从存储介质中读取
	...
	// 一个NAT区域所包含的block数目，在mkfs的时候已经确定
	unsigned int nat_blocks;	/* # of nat blocks */

	/* free node ids management */
	// TODO：通过红黑树缓存free nid，为了加快查找速度？
	struct radix_tree_root free_nid_root;/* root of the free_nid cache */
	// TODO：缓存一些已经被释放的node block？
	struct list_head free_nid_list;		/* list for free nids excluding preallocated nids */
	unsigned int nid_cnt[MAX_NID_STATE];	/* the number of free node id */
	spinlock_t nid_list_lock;	/* protect nid lists ops */
	struct mutex build_lock;	/* lock for build free nids */
	// bitmap: 记录data区域每个node是否被使用（或者说每个node id是否被占用）
	// 注意：free_nid_bitmap要配合nat_block_bitmap使用
